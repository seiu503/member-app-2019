...
// chai.use(chaiHttp);
// let authenticateMock, userStub, multerStub, s3;
...
//     // sendStub = sinon.stub(upload, 'send').callsFake((callback) =>
//     //   callback());
...
//     test.only("uploads an image to AWS S3 bucket", function(done) {
//       // authenticateMock = sinon.stub(passport, "authenticate").returns(() => {});
//       // const user = [{ name, email, avatar_url, google_token, google_id }];
//       // userStub = sinon.stub(users, "createUser").resolves(user);
//       // authenticateMock.yields(null, {
...
//       // dataStream.push(null);
//       // updates = {};
//       uploadStub = sinon.stub(upload).returns(multerStub);
//       chai
//         .request(app)
...
//           assert.isNull(err);
//           sinon.assert.called(multerStub);
//           authenticateMock.restore();
//           userStub.restore();
//           multerStub.restore();
//           aws.S3.restore();
